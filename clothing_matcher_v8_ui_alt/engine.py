...
APP_DATA_FOLDER = "ClothingMatcherV8"
INDEX_SCHEMA = "v8-regions-2"
...
class EmbeddingEngine:
        ...
        if model_path and model_path.exists():
            try:
                ...
                if "sscd" in model_path.name.lower():
                    ...
                    self.mode = "SSCD 四视角 + 纹理 + OCR文字融合 V8（CPU）"
    ...
    def _embed_sscd(self, path: Path) -> np.ndarray:
        prepared = self._prepare_sscd(path)
        input_name = self.session.get_inputs()[0].name
        learned = np.asarray(
            self.session.run(None, {input_name: prepared})[0],
            dtype=np.float32,
        )
        return self._combine_sscd(learned, path)

    def _prepare_sscd(self, path: Path) -> np.ndarray:
        with Image.open(path) as src:
            base = _crop_garment(ImageOps.exif_transpose(src).convert("RGB"))
        w, h = base.size
        print_crop = base.crop((int(.08 * w), int(.16 * h), int(.92 * w), int(.82 * h)))
        # Small chest logos were the main source of false negatives in V7.  A
        # dedicated upper-garment view keeps those details large enough for SSCD,
        # while the full and center views still cover large/all-over prints.
        upper_crop = base.crop((int(.10 * w), int(.04 * h), int(.90 * w), int(.58 * h)))
        chest_crop = base.crop((int(.42 * w), int(.13 * h), int(.92 * w), int(.52 * h)))
        prepared = []
        mean = np.asarray([0.485, 0.456, 0.406], dtype=np.float32)
        std = np.asarray([0.229, 0.224, 0.225], dtype=np.float32)
        for im in (base, print_crop, upper_crop, chest_crop):
    ...
    def _combine_sscd(self, learned: np.ndarray, path: Path) -> np.ndarray:
        learned = learned.astype(np.float32, copy=False)
        learned /= np.linalg.norm(learned, axis=1, keepdims=True) + 1e-8
        texture = _classic_descriptor(path)
        combined = np.concatenate(
            [
                learned[0] * np.sqrt(0.25),
                learned[1] * np.sqrt(0.15),
                learned[2] * np.sqrt(0.26),
                learned[3] * np.sqrt(0.28),
                texture * np.sqrt(0.06),
            ]
        ).astype(np.float32)
    ...
    def embed_many(self, paths: Sequence[Path], batch_size: int = 8) -> list[np.ndarray]:
        if self.session is None or self.model_kind not in {"dinov2", "sscd"}:
            return [self.embed(path) for path in paths]
        vectors: list[np.ndarray] = []
        input_name = self.session.get_inputs()[0].name
        if self.model_kind == "sscd":
            # Run all four views for a small group in one ONNX call.  This keeps
            # V8's richer descriptor faster than V7's per-image two-view calls.
            for start in range(0, len(paths), batch_size):
                batch_paths = paths[start:start + batch_size]
                arrays = np.concatenate([self._prepare_sscd(path) for path in batch_paths], axis=0)
                learned_batch = np.asarray(self.session.run(None, {input_name: arrays})[0])
                learned_batch = learned_batch.reshape(len(batch_paths), 4, -1)
                vectors.extend(
                    self._combine_sscd(learned, path)
                    for learned, path in zip(learned_batch, batch_paths)
                )
            return vectors
        for start in range(0, len(paths), batch_size):
            batch_paths = paths[start:start + batch_size]
...
class LibraryIndex:
    ...
    def cache_path(self) -> Path:
        identity = f"{INDEX_SCHEMA}|{self.engine.mode}|{self.library}"
    ...
    def feedback_path(self) -> Path:
        identity = f"{INDEX_SCHEMA}|{self.engine.mode}|{self.library}"
        key = hashlib.sha1(identity.encode("utf-8")).hexdigest()[:16]
        return app_data_dir() / f"feedback-{key}.npz"

    @staticmethod
    def _query_key(path: Path) -> str:
        return f"{path.resolve()}|{path.stat().st_mtime_ns}"

    def _load_feedback(self) -> tuple[list[str], list[str], np.ndarray]:
        if not self.feedback_path.exists():
            return [], [], np.empty((0, 0), dtype=np.float32)
        try:
            data = np.load(self.feedback_path, allow_pickle=False)
            keys = [str(value) for value in data["keys"].tolist()]
            selected = [str(value) for value in data["selected"].tolist()]
            vectors = data["vectors"].astype(np.float32)
            if vectors.ndim != 2 or len(keys) != len(selected) or len(keys) != len(vectors):
                raise ValueError("invalid feedback cache")
            return keys, selected, vectors
        except Exception:
            return [], [], np.empty((0, 0), dtype=np.float32)

    def record_feedback(self, query_path: Path, selected_path: Path) -> None:
        """Remember a human-confirmed real photo as a local style exemplar."""
        if selected_path not in self.paths:
            return
        key = self._query_key(query_path)
        vector = self._query_cache.get(key)
        if vector is None:
            vector = self.engine.embed(query_path)
            self._query_cache[key] = vector
        keys, selected, vectors = self._load_feedback()
        if vectors.size and vectors.shape[1] != vector.size:
            keys, selected, vectors = [], [], np.empty((0, vector.size), dtype=np.float32)
        if key in keys:
            index = keys.index(key)
            selected[index] = str(selected_path.resolve())
            vectors[index] = vector
        else:
            keys.append(key)
            selected.append(str(selected_path.resolve()))
            vectors = np.vstack([vectors, vector[None, :]]) if vectors.size else vector[None, :]
        # Bound long-running shops to a compact, fast local memory.
        keys, selected, vectors = keys[-500:], selected[-500:], vectors[-500:]
        np.savez_compressed(
            self.feedback_path,
    ...
    def build(self, progress: Callable[[int, int, str], None] | None = None) -> int:
        ...
        vectors: list[np.ndarray] = []
        texts: list[str] = []
        text_cache = _load_design_text_cache()
        text_cache_changed = False
        for start in range(0, len(paths), 8):
            ...
            for path in batch_paths:
                cache_key = _design_text_cache_key(path)
                if cache_key not in text_cache:
                    text_cache[cache_key] = detect_design_text(path)
                    text_cache_changed = True
                texts.append(text_cache[cache_key])
        ...
        if text_cache_changed:
            _save_design_text_cache(text_cache)
        return len(paths)
    ...
    def search(self, query_path: Path, top_k: int = 5) -> list[Match]:
        ...
        self._query_cache[self._query_key(query_path)] = query
        ...
        # Human confirmations become exemplars.  Only a reasonably similar
        # exemplar contributes, and it boosts its confirmed catalog item rather
        # than globally distorting every candidate score.
        _, feedback_selected, feedback_vectors = self._load_feedback()
        if feedback_vectors.size and feedback_vectors.shape[1] == query.size:
            feedback_scores = feedback_vectors @ query
            path_to_index = {str(path.resolve()): index for index, path in enumerate(self.paths)}
            for selected_path in set(feedback_selected):
                relevant = [
                    float(score)
                    for score, path in zip(feedback_scores, feedback_selected)
                    if path == selected_path
                ]
                similarity = max(relevant, default=-1.0)
                catalog_index = path_to_index.get(selected_path)
                if catalog_index is not None and similarity >= 0.32:
                    scores[catalog_index] += min(0.22, (similarity - 0.20) * 0.55)
...
def _design_text_cache_key(path: Path) -> str:
    return f"{path.resolve()}|{path.stat().st_mtime_ns}"


def _load_design_text_cache() -> dict[str, str]:
    cache_path = app_data_dir() / "design_text_cache.json"
    ...
    # Reuse the OCR work already completed by V7, without sharing or altering
    # V7's index file.  This makes the first V8 index substantially faster.
    legacy_dir = Path(os.environ.get("LOCALAPPDATA", Path.home())) / "ClothingMatcher"
    migrated: dict[str, str] = {}
    for index_path in legacy_dir.glob("index-*.npz") if legacy_dir.exists() else []:
        try:
            data = np.load(index_path, allow_pickle=False)
            if "texts" not in data:
                continue
            for raw_path, mtime, text in zip(
                data["paths"].tolist(), data["mtimes"].tolist(), data["texts"].tolist()
            ):
                migrated[f"{Path(raw_path).resolve()}|{int(mtime)}"] = str(text)
        except Exception:
            continue
    return migrated


def _save_design_text_cache(cache: dict[str, str]) -> None:
    path = app_data_dir() / "design_text_cache.json"
...
def detect_red_text_tokens(path: Path) -> tuple[list[str], str]:
    ...
    try:
        ...
        red = (
            (arr[:, :, 0] > 145)
            & (arr[:, :, 0].astype(np.float32) > arr[:, :, 1] * 1.18)
            & (arr[:, :, 0].astype(np.float32) > arr[:, :, 2] * 1.12)
            & (
                arr[:, :, 0].astype(np.int16)
                - np.maximum(arr[:, :, 1], arr[:, :, 2]).astype(np.int16)
                > 18
            )
        )
        ...
        red_text = ""
        if len(xs) >= 30:
            # Crop tightly around the annotation. Tiny one-character labels such
            # as “绿” are frequently ignored when OCR sees the entire portrait.
            pad = 30
            x0, x1 = max(0, int(xs.min()) - pad), min(image.width, int(xs.max()) + pad + 1)
            y0, y1 = max(0, int(ys.min()) - pad), min(image.height, int(ys.max()) + pad + 1)
            red_crop = image.crop((x0, y0, x1, y1))
            result, _ = _OCR_ENGINE(np.asarray(red_crop))
            ...
            if not red_text.strip():
                ...
                red_text = "\n".join(str(row[1]) for row in (result or []))
        tokens = parse_size_tokens(red_text)
        if not tokens:
            result, _ = _OCR_ENGINE(np.asarray(image))
            full_text = "\n".join(row[1] for row in (result or []))
            tokens = parse_size_tokens(full_text)
        # Color labels must come from the red handwritten/typed annotation when
        # it is available. Returning the full-shirt OCR here could mistake a
        # design word for an operator-entered color.
        return tokens, red_text
    except Exception:
        return [], ""


def parse_marked_color_text(text: str) -> str:
    """Extract an explicitly written Chinese color, preferring specific names."""
    ...
    aliases = (
        ...
        ("橙色", "橙色"), ("咖色", "咖色"), ("棕色", "棕色"),
        ("杏色", "杏色"), ("驼色", "驼色"),
        # RapidOCR commonly reads a large standalone red “绿” as Cyrillic
        # “бл”; accept that narrow, observed confusion only inside red labels.
        ("бл", "绿色"), ("БЛ", "绿色"),
        ("黑", "黑色"), ("白", "白色"), ("灰", "灰色"),
        ("绿", "绿色"), ("蓝", "蓝色"), ("红", "红色"),
        ("紫", "紫色"), ("粉", "粉色"), ("黄", "黄色"),
    )
    for marker, color in aliases:
...
def detect_color_name(path: Path) -> str:
    ...
    if saturation < 0.13:
        ...
        if value < 0.55:
            return "黑色"
        # Off-white garments commonly fall near 0.80 under indoor shop lighting;
        # keeping the cutoff at 0.82 mislabeled them as gray in V7.
        if value < 0.80:
            return "灰色"
        return "白色"
...
def annotate_result(
    ...
    # Result previews are often checked on a small screen.  Keep the red stock
    # information clearly readable after the image is fitted into the canvas.
    # Also fit a long color + multi-size list inside landscape catalogue photos.
    fit_by_height = max(52, int(im.height / max(1, len(labels)) / 1.30))
    font_size = max(52, min(150, im.width // 8, fit_by_height))
...
def load_settings() -> dict:
    path = app_data_dir() / "settings.json"
    if not path.exists():
        # First V8 launch inherits only the harmless paths from V7.  All future
        # settings, indexes and previews remain isolated in ClothingMatcherV8.
        legacy = Path(os.environ.get("LOCALAPPDATA", Path.home())) / "ClothingMatcher" / "settings.json"
        if legacy.exists():
            try:
                data = json.loads(legacy.read_text(encoding="utf-8"))
                return {key: data.get(key, "") for key in ("library", "output")}
            except Exception:
                pass
        return {}
    try:
